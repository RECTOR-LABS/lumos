...
This file holds ecosystem-wide context for AI agents working across all LUMOS
repositories, plus core-repository-specific detail (the `getlumos/lumos` repo is
the schema compiler + CLI + LSP server).
...
These standards apply to ALL repositories under the getlumos organization.
...
- Maintain repo-specific AGENTS.md (CLAUDE.md retained for backward compat, references this file)
...
- Check repo-specific AGENTS.md (or CLAUDE.md) files for individual development notes

---

# LUMOS Core Repository

> The sections below are specific to the `getlumos/lumos` repository (core compiler and CLI). For other repos, see their respective context files linked above.
